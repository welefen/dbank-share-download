...
share.prototype = {
		...
		return this.getContent().then(function(content){
			...
			function getUrl(files, dest){
				...
				files.forEach(function(item){
					...
					if (downloadurl) {
						result.push({
							url: self.decrypt(downloadurl, encryKey),
							dest: dest
						});
					}else if(item.childList && item.type === "Directory"){
						var childResult = getUrl(item.childList, path.join(dest, item.name));
	 ...
	 * 		dest: ""
	 * }]
	 * @param  {[type]} urlsInfo [description]
	 * @return {[type]}          [description]
	 */
	download: function(urlsInfo){
	    var promises = [];
	    var self = this;
		urlsInfo.forEach(function(item){
			var options = {
				url: item.url,
				headers: share.headers
			};
			...
			req.on("response", function(res){
				...
				var filepath = path.join(item.dest, path.basename(item.url));
				...
				if (disposition && disposition.indexOf('attachment') > -1) {
					...
					if (disposition) {
						filepath = path.join(item.dest, disposition);
					};
				};
